...
# water molecules (default 10), by calling `sbatch submit_macepbe0.sh` once
# per matching file. Run this directly on the login node (it is not itself
# an sbatch script).
#
# Usage: ./submit_all_macepbe0.sh [--xyz-dir DIR] [--pattern PATTERN] [--step N] [--max N] [-- extra benchmark_mace.py flags]
# e.g.:  ./submit_all_macepbe0.sh
# e.g.:  ./submit_all_macepbe0.sh --step 20 --max 200
# e.g.:  ./submit_all_macepbe0.sh -- --pbe0-checkpoint /path/to/pbe0.model
set -euo pipefail
...
echo "Submitting MACEPBE0 jobs at increments of ${STEP} waters (up to ${MAX})..."
for (( n = STEP; n <= MAX; n += STEP )); do
    if [[ -n "${FILE_FOR_N[$n]:-}" ]]; then
        echo "  ${n} waters -> ${FILE_FOR_N[$n]}"
        sbatch "${SCRIPT_DIR}/submit_macepbe0.sh" "${FILE_FOR_N[$n]}" "${EXTRA_ARGS[@]}"
    else
