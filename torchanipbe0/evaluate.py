...
class EnsembleEvaluater:
    def __init__(self, info_file_path, dtype, device, unit = None):
        ...
        for i in range(ensemble_size):
            ...
            elif info_file_path == 'ani-pbe0-2x_8x.info':
                network_dir = f'{ensemble_prefix}{i}'
                self.neural_networks.append(neurochem.load_model_3(network_dir, aev_dim).to(device))
            else:
...
def ani_pbe0_2x_evaluater(device = torch.device('cpu'), unit = None):
    return EnsembleEvaluater('ani-pbe0-2x_8x.info', torch.float32, device, unit)
